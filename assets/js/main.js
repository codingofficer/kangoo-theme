...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupCheckoutMultistep() {
    ...
    const checkout = document.querySelector('.woocommerce-checkout');
    const main = checkout ? checkout.querySelector('.wc-block-checkout__main, .wc-block-components-main, .wp-block-woocommerce-checkout-fields-block') : null;
    ...
    checkout.classList.add('kangoo-checkout-native-flow');

    const contactBlock = main.querySelector('.wp-block-woocommerce-checkout-contact-information-block');
    const storedDob = getStoredCheckoutDob() || { day: '', month: '', year: '' };
    let dobPanel = checkout.querySelector('[data-kangoo-checkout-dob-panel]');

    if (!dobPanel) {
      dobPanel = document.createElement('div');
      dobPanel.className = 'kangoo-checkout-dob-panel';
      dobPanel.setAttribute('data-kangoo-checkout-dob-panel', '');
      dobPanel.innerHTML = [
        '<strong>Date of birth</strong>',
        '<p>Required for age-restricted products. Photo ID verification happens in the next step.</p>',
        '<div class="kangoo-checkout-dob-panel__fields" data-kangoo-checkout-dob>',
        '<label><span>DD</span><input type="text" inputmode="numeric" maxlength="2" autocomplete="bday-day" data-kangoo-cart-dob-day></label>',
        '<label><span>MM</span><input type="text" inputmode="numeric" maxlength="2" autocomplete="bday-month" data-kangoo-cart-dob-month></label>',
        '<label><span>YYYY</span><input type="text" inputmode="numeric" maxlength="4" autocomplete="bday-year" data-kangoo-cart-dob-year></label>',
        '</div>',
        '<p class="kangoo-checkout-dob-panel__message" data-kangoo-checkout-details-message aria-live="polite"></p>'
      ].join('');

      if (contactBlock) {
        contactBlock.insertAdjacentElement('afterend', dobPanel);
      } else {
        main.insertAdjacentElement('afterbegin', dobPanel);
      }
    }

    const dobDay = dobPanel.querySelector('[data-kangoo-cart-dob-day]');
    const dobMonth = dobPanel.querySelector('[data-kangoo-cart-dob-month]');
    const dobYear = dobPanel.querySelector('[data-kangoo-cart-dob-year]');

    if (storedDob && !dobDay.value && !dobMonth.value && !dobYear.value) {
      dobDay.value = storedDob.day || '';
      dobMonth.value = storedDob.month || '';
      dobYear.value = storedDob.year || '';
    }

    const emailField = getCheckoutEmailField(checkout);
    const savedEmail = getStoredCheckoutEmail();
    if (savedEmail && emailField && !emailField.value) {
      setCheckoutStepInputValue(emailField, savedEmail);
    }
    ...
    const stepGroups = {
      details: [
        contactBlock,
        dobPanel,
        checkout.querySelector('[data-kangoo-checkout-guest-notice]'),
        main.querySelector('.wp-block-woocommerce-checkout-shipping-address-block'),
        main.querySelector('.wp-block-woocommerce-checkout-billing-address-block'),
        main.querySelector('.wp-block-woocommerce-checkout-shipping-method-block'),
        main.querySelector('.wp-block-woocommerce-checkout-pickup-options-block'),
        main.querySelector('.wp-block-woocommerce-checkout-shipping-methods-block')
      ],
      verify: [],
      pay: [
        main.querySelector('.wp-block-woocommerce-checkout-express-payment-block'),
        main.querySelector('.wp-block-woocommerce-checkout-payment-block'),
        main.querySelector('.wp-block-woocommerce-checkout-additional-information-block'),
        main.querySelector('.wp-block-woocommerce-checkout-order-note-block'),
        main.querySelector('.wp-block-woocommerce-checkout-terms-block'),
        main.querySelector('.wp-block-woocommerce-checkout-actions-block')
      ]
    };

    let verifySummary = checkout.querySelector('[data-kangoo-checkout-verify-summary]');
    if (!verifySummary) {
      verifySummary = document.createElement('div');
      verifySummary.className = 'kangoo-checkout-verify-summary';
      verifySummary.setAttribute('data-kangoo-checkout-verify-summary', '');
      dobPanel.insertAdjacentElement('afterend', verifySummary);
    }

    const verificationCard = checkout.querySelector('[data-kangoo-av]');
    let avPlaceholder = checkout.querySelector('[data-kangoo-av-placeholder]');

    if (verificationCard && avPlaceholder) {
      avPlaceholder.remove();
      avPlaceholder = null;
    }

    if (!verificationCard && !avPlaceholder) {
      avPlaceholder = document.createElement('div');
      avPlaceholder.className = 'kangoo-av kangoo-av--placeholder';
      avPlaceholder.setAttribute('data-kangoo-av-placeholder', '');
      avPlaceholder.innerHTML = '<div class="kangoo-av__content"><span class="kangoo-av__eyebrow">Required before payment</span><h3>Photo ID verification</h3><p>The secure selfie check will appear here when Stripe Identity enforcement is enabled.</p></div>';
      verifySummary.insertAdjacentElement('afterend', avPlaceholder);
    }

    stepGroups.verify = [verifySummary, verificationCard || avPlaceholder];

    Object.keys(stepGroups).forEach(function (stepKey) {
      stepGroups[stepKey].filter(Boolean).forEach(function (element) {
        element.setAttribute('data-kangoo-checkout-native-step', stepKey);
      });
    });

    function ensureActions(stepKey, afterElement) {
      if (!afterElement) {
        return null;
      }

      let actions = checkout.querySelector('[data-kangoo-step-actions="' + stepKey + '"]');

      if (!actions) {
        actions = document.createElement('div');
        actions.className = 'kangoo-checkout-step-actions';
        actions.setAttribute('data-kangoo-step-actions', stepKey);
        actions.setAttribute('data-kangoo-checkout-native-step', stepKey);
        actions.innerHTML = stepKey === 'details'
          ? '<button type="button" class="kangoo-checkout-step-actions__next" data-kangoo-step-next="verify">Continue to verification</button>'
          : '<button type="button" class="kangoo-checkout-step-actions__back" data-kangoo-step-back="details">Back</button><button type="button" class="kangoo-checkout-step-actions__next" data-kangoo-step-next="pay">Continue to payment</button>';
        afterElement.insertAdjacentElement('afterend', actions);
      }

      return actions;
    }

    const detailActions = ensureActions('details', stepGroups.details.filter(Boolean).slice(-1)[0]);
    const verifyActions = ensureActions('verify', stepGroups.verify.filter(Boolean).slice(-1)[0]);
    ...
    function getCheckoutStepDob() {
      return normalizeCheckoutDob(dobDay.value, dobMonth.value, dobYear.value);
    }

    function setDetailsMessage(text, isError) {
      const message = dobPanel.querySelector('[data-kangoo-checkout-details-message]');
      if (!message) {
        return;
      }
      message.textContent = text || '';
    ...
    function updateVerifySummary() {
      const email = emailField ? emailField.value : getStoredCheckoutEmail();
      const dob = getCheckoutStepDob();
      const summaryHtml = [
        '<strong>Details ready</strong>',
        '<span>', escapeHtml(isValidEmail(email) ? email : 'Email required'), '</span>',
        '<span>', escapeHtml(isValidCheckoutDob(dob) ? formatCheckoutDobDisplay(dob) : 'Date of birth required'), '</span>'
      ].join('');

      if (verifySummary.innerHTML !== summaryHtml) {
        verifySummary.innerHTML = summaryHtml;
      }
    }

    function validateAddressFields() {
      const fields = stepGroups.details.filter(Boolean).reduce(function (all, element) {
        return all.concat(Array.from(element.querySelectorAll('input[required], select[required], textarea[required], [aria-required="true"]')));
      }, []);

      return fields.filter(function (field) {
        return field.offsetParent !== null && !field.disabled && field.type !== 'hidden';
    ...
    function validateShippingMethod() {
      const shippingInputs = getDeliveryShippingInputs().filter(function (input) {
        return stepGroups.details.some(function (element) {
          return element && element.contains(input);
        }) && input.offsetParent !== null && !input.disabled;
      });

      return !shippingInputs.length || shippingInputs.some(function (input) {
        return input.checked;
      });
    }

    function focusFirstInvalidDetail() {
      const dob = getCheckoutStepDob();
      const email = emailField ? emailField.value : '';

      if (!isValidEmail(email) && emailField) {
        emailField.focus();
        return;
      }
      ...
      const invalidField = stepGroups.details.filter(Boolean).reduce(function (found, element) {
        return found || Array.from(element.querySelectorAll('input[required], select[required], textarea[required], [aria-required="true"]')).find(function (field) {
          return field.offsetParent !== null && !field.disabled && field.type !== 'hidden' && String(field.value || '').trim() === '';
        });
      }, null);
    ...
    function setActiveStep(stepKey, shouldScroll) {
      checkout.dataset.kangooActiveStep = stepKey;
      renderCheckoutProgress(stepKey);
      checkout.querySelectorAll('[data-kangoo-checkout-native-step]').forEach(function (element) {
        element.hidden = element.getAttribute('data-kangoo-checkout-native-step') !== stepKey;
      });
      updateVerifySummary();
      window.sessionStorage.setItem('kangooCheckoutActiveStep', stepKey);

      if (shouldScroll) {
        (checkout.querySelector('[data-kangoo-checkout-progress]') || main).scrollIntoView({ behavior: 'smooth', block: 'start' });
      }
    }

    [dobDay, dobMonth, dobYear].forEach(function (field) {
      if (field.dataset.kangooCheckoutDobReady === '1') {
        return;
      }
      field.dataset.kangooCheckoutDobReady = '1';
      field.addEventListener('input', function () {
        cleanDobInput(field);
        if (field === dobDay && field.value.length >= 2) {
          dobMonth.focus();
      ...
      field.addEventListener('blur', function () {
        cleanDobInput(field);
        if (field !== dobYear && field.value.length === 1) {
          field.value = field.value.padStart(2, '0');
        }
        updateVerifySummary();
      });
    ...
    if (checkout.dataset.kangooStepClicksReady !== '1') {
      checkout.dataset.kangooStepClicksReady = '1';
      checkout.addEventListener('click', function (event) {
        const back = event.target.closest('[data-kangoo-step-back]');
        const next = event.target.closest('[data-kangoo-step-next]');

        if (back) {
          setActiveStep(back.getAttribute('data-kangoo-step-back') || 'details', true);
          return;
        }
        ...
        const targetStep = next.getAttribute('data-kangoo-step-next');

        if (targetStep === 'verify') {
          const email = emailField ? emailField.value : '';
          const dob = getCheckoutStepDob();

          if (!isValidEmail(email) || !isValidCheckoutDob(dob) || !validateAddressFields() || !validateShippingMethod()) {
            setDetailsMessage('Complete your email, date of birth, address and delivery option first.', true);
            focusFirstInvalidDetail();
            return;
          }

          next.disabled = true;
          saveCheckoutEmail(email, dob).then(function (payload) {
            setDetailsMessage('Details saved.', false);
            if (window.kangooRewards) {
              window.kangooRewards.checkout_email = payload.email;
              window.kangooRewards.checkout_dob = payload.dob;
              window.kangooRewards.checkout_email_is_existing_customer = Boolean(payload.existingCustomer);
            }
            if (payload.removedFirstOrderCoupon) {
              window.location.reload();
              return;
            }
            setActiveStep('verify', true);
          }).catch(function (error) {
            setDetailsMessage(error.message || 'Could not save your checkout details.', true);
            focusFirstInvalidDetail();
          }).finally(function () {
            next.disabled = false;
          });
        ...
        if (targetStep === 'pay') {
          if (!isCheckoutVerificationComplete()) {
            const card = checkout.querySelector('[data-kangoo-av]');
            if (card) {
              card.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
            return;
          }
          setActiveStep('pay', true);
        }
      });
    }

    if (detailActions) {
      detailActions.setAttribute('data-kangoo-checkout-native-step', 'details');
    }
    if (verifyActions) {
      verifyActions.setAttribute('data-kangoo-checkout-native-step', 'verify');
    }

    const storedStep = window.sessionStorage.getItem('kangooCheckoutActiveStep');
    const initialStep = storedStep && ['details', 'verify', 'pay'].includes(storedStep) ? storedStep : 'details';
    setActiveStep(initialStep === 'pay' && !isCheckoutVerificationComplete() ? 'verify' : initialStep, false);
  }
