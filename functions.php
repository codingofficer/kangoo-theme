...
function kangoo_async_style_loader_tag($html, $handle, $href, $media) {
    $is_home_cookie_blocker_style = is_front_page()
        && is_string($href)
        && strpos($href, 'cookieblocker.min.css') !== false;

    if (!kangoo_should_async_style($handle) && !$is_home_cookie_blocker_style) {
        return $html;
    }
...
function kangoo_move_homepage_jquery_to_footer() {
...
add_action('wp_enqueue_scripts', 'kangoo_move_homepage_jquery_to_footer', 9999);
...
function kangoo_render_cart_secure_checkout_banner() {
    ...
    $image_url = get_theme_file_uri('/assets/images/secure-checkout-worldpay.png');
    ?>
    <aside class="kangoo-cart-secure-checkout" aria-label="<?php esc_attr_e('Secure checkout powered by Worldpay', 'kangoo'); ?>">
        <img src="<?php echo esc_url($image_url); ?>" width="300" height="107" alt="<?php esc_attr_e('Secure checkout powered by Worldpay', 'kangoo'); ?>">
    </aside>
    <?php
}
